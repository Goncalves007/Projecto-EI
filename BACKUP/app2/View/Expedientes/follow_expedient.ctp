...
<?php }?> 
											...
											<?php foreach ($confirmas as $confirma) { ?>
												...
												<tr class="pure-table-odd">
                                                    <td><?php echo $confirma['Expediente']['nr_expediente']; ?></td>
                                                    <td><?php echo $confirma['Expediente']['nr_guia']; ?></td>
                                                    <td><?php echo $confirma['Office']['nome']; ?></td>
                                                    <td><?php echo $confirma['User']['username']; ?></td>
                                                    <td><?php echo $confirma['Motorista']['name']; ?></td>
                                                    <td>
                                                    <?php if(!empty($confirma['Correio']['name'])){
                                                      echo $confirma['Correio']['name'];
                                                    }else{
                                                      echo "------";
                                                    }
                                                     ?>
        ...
        if (($confirma['Expediente']['status'] == 0)) { ?>
            <td><em style='color:#000080'><b><?php echo $this->Html->link('Nao Confirmado', array('controller' => 'expedientes', 'action' =>'confirmar',$confirma['Expediente']['id'])) ?>  </b></em></td>
        <?php }elseif($confirma['Expediente']['status'] == 1){ ?>
            <td><em style='color:#800000'><b><?php echo $this->Html->link('Confirmado', array('controller' => 'expedientes', 'action' =>'confirmar',$confirma['Expediente']['id'])) ?></b></em></td>
        <?php } ?>
                                                </tr>
                                                <?php } ?>										
                                            </tbody>
